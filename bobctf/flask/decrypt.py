...
def get_signing_serializer(key):
	signer_kwargs = dict(
		key_derivation='hmac',
...
with open(session_fname, 'rb') as f:
	sdata = f.read()

with open(key_fname, 'rb') as f:
	key = f.read()

def open_session(key, cipher):
	s = get_signing_serializer(key)
	plain = s.loads(cipher)
	return plain


# print (key)
print (open_session(key, sdata))
